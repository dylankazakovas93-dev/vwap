"""Post-touch excursion outcomes and barrier-first outcomes, bounded
strictly within one session-leg instance's local bar array.

See SPEC_SESSION_HORIZONS.md sections 6, 8-10.
"""
import numpy as np
import pandas as pd
...
def add_outcomes(local_bars: pd.DataFrame, touches: pd.DataFrame) -> pd.DataFrame:
    if touches.empty:
        return touches
    low = local_bars["low"].to_numpy()
    high = local_bars["high"].to_numpy()
    close = local_bars["close"].to_numpy()
    atr_event_col = local_bars["atr_event"].to_numpy()
    n = len(local_bars)

    rows = []
    for ev in touches.itertuples():
        T = ev.local_touch_idx
        ...
        for H in HORIZONS:
            ...
            if not complete:
                for key in (
                    "rejection_exc", "breakthrough_exc", "signed_close",
                    "rejection_exc_atr", "breakthrough_exc_atr",
                    "signed_close_atr", "dominance",
                ):
...
def add_barrier_outcomes(local_bars: pd.DataFrame, events_out: pd.DataFrame) -> pd.DataFrame:
    if events_out.empty:
        return events_out
    low = local_bars["low"].to_numpy()
    high = local_bars["high"].to_numpy()
    ...
    for ev in events_out.itertuples():
        ...
        level_t = ev.level_t
        atr = ev.atr_event
        row = {"local_touch_idx": T, "instrument": ev.instrument,
               "session_date": ev.session_date, "session": ev.session}
        atr_valid = np.isfinite(atr) and atr > 0
        for b in BARRIERS:
            for H in HORIZONS:
                key = f"b{b}_h{H}"
