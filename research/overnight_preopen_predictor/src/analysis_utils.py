"""Self-contained statistical utilities for generation 5."""
import hashlib
import json
import os
import subprocess
...
GEN_DIR = os.path.join(REPO, "research", "overnight_preopen_predictor")
...
class Registry:
    ...
    def log(self, hyp, config, status="ok", metric="", notes=""):
        ...
        row = pd.DataFrame([{
            "run_id": f"g5_{self.n:04d}", "timestamp_utc": datetime.now(timezone.utc).isoformat(),
            "git_sha": self.sha, "dataset_hash": "see DATA_CONTRACT.md", "stage": "overnight_screen",
